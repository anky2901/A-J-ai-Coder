...
`,
          ...
          try {
            ...
            const secondMessageStart = Date.now();
            await client.workspace.sendMessage({
              workspaceId: workspaceId!,
              message: "What is 2 + 2?",
              options: {
            ...
            // Wait for stream to complete
            await collector.waitForEvent("stream-end", streamTimeout);
            const secondMessageEnd = Date.now();
            const secondMessageDuration = secondMessageEnd - secondMessageStart;

            collector.stop();

            // Second message should be faster than first (no init wait)
            // First message includes ~2s init wait, second doesn't
            // Second should be significantly faster (at least 1 second faster)
            expect(firstMessageDuration - secondMessageDuration).toBeGreaterThan(1000);
          } finally {
            await cleanupTestEnvironment(env);
            await cleanupTempGitRepo(repoPath);
          }
